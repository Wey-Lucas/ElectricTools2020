...
Namespace Engine2
    ...
    ''' <summary>
    ''' Armazena a coleção de itens relacionados a mudança de entidades
    ''' </summary>
    ''' <remarks></remarks>
    Public NotInheritable Class ChangedProcessEventArgs : Inherits List(Of ChangedProcess) : Implements IDisposable
...
#End Region
        ...
        ''' <summary>
        ''' Adiciona item (Não permite replicas)
        ''' </summary>
            ...
            Try
                ...
                For Each ObjectId As ObjectId In ObjectIds
                    ...
                    If Me.ValidId(ObjectId) = True Then
                        ...
                        If IsNothing(MyBase.Find(Function(X As ChangedProcess) X.ObjectId.Equals(ObjectId) = True)) = True Then

                            'Adiciona o item
                            Me.Add(New ChangedProcess(Me._Reactors, ObjectId))
        ...
        Private Sub RemoveObject(ObjectIds() As ObjectId)
            Try
                ...
                'Remove os itens
                Me.RemoveAll(Function(X As ChangedProcess) ArrayList.Contains(X.ObjectId) = True)
        ...
        Private Function ValidId(ObjectId As ObjectId) As Boolean
            Try
                ...
                If IsNothing(Me._Reactors.Find(Function(X As Reactor) X.ObjectId.Equals(ObjectId) = True And X.Active = True And X.ReactorEvents.Contains("ChangeReaction") = True)) = False Then
        ...
        ''' <param name="NewState">Entidade em seu estado atual</param>
        ''' <param name="PreventDuplication">Impede a entrada de duplicações</param>
        ''' <remarks></remarks>
        Public Overloads Sub Add(OriginalState As Entity, NewState As Entity, Optional PreventDuplication As Boolean = True)
            Try
                ...
                If PreventDuplication = True Then
                    ...
                    'Verifica se existe
                    If Me.Contains(OriginalState, NewState) = False Then

                        'Adiciona
                        MyBase.Add(New ChangedProcess(Me._Reactors, OriginalState, NewState))
        ...
        ''' <summary>
        ''' Retorna a coleção dos Ids atuais
        ''' </summary>
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function GetNewStateObjectIds() As ObjectIdCollection
            Try

                'Inicializa a coleção
                GetNewStateObjectIds = New ObjectIdCollection

                'Percorre a coleção
                For Index As Integer = 0 To MyBase.Count - 1

                    'Adiciona a entidade
                    GetNewStateObjectIds.Add(MyBase.Item(Index).NewState.ObjectId)

                Next

                'Retorno
                Return GetNewStateObjectIds
        ...
        ''' <summary>
        ''' Retorna a coleção das entidades atuais
        ''' </summary>
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function GetNewStateEntitys() As DBObjectCollection
            Try

                'Declarações
                GetNewStateEntitys = New DBObjectCollection

                'Percorre a coleção
                For Index As Integer = 0 To MyBase.Count - 1

                    'Adiciona a entidade
                    GetNewStateEntitys.Add(MyBase.Item(Index).NewState)

                Next

                'Retorno
                Return GetNewStateEntitys
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function IsTypeEquals(Optional Match As System.Predicate(Of ChangedProcess) = Nothing) As Boolean
            Try
        ...
        ''' </summary>
        ''' <param name="OriginalState">Estado original da entidade</param>
        ''' <param name="NewState">Estado atual da entidade</param>
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Overloads Function Contains(OriginalState As Entity, Optional NewState As Entity = Nothing) As Boolean
            Try

                'Avalia NewState
                If IsNothing(NewState) = True Then

                    'Verifica se existe
                    If IsNothing(MyBase.Find(Function(X As ChangedProcess) X.OriginalState.Equals(OriginalState) = True)) = True Then
                ...
                Else
                    ...
                    'Verifica se existe
                    If IsNothing(MyBase.Find(Function(X As ChangedProcess) X.OriginalState.Equals(OriginalState) = True And X.NewState.Equals(NewState) = True)) = True Then
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Class ChangedProcess : Inherits ProcessBase
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public ReadOnly Property NewState As Entity
            Get
                Me.GetNewState()
                Return Me._NewState
            End Get
        ...
        ''' <summary>
        ''' Atualiza o estado atual da entidade
        ''' </summary>
        ''' <remarks></remarks>
        Private Sub GetNewState()
            Try

                'Obtem o estado atual do item
                Me._NewState = Engine2.ConvertObject.ObjectIDToDBObject(Me.ObjectId)
        ...
        Public Sub New(ByRef Reactors As Reactors, ObjectId As ObjectId)
            ...
            'Armazena a classe proprietária
            Me._Reactors = Reactors

            'Seta NewState
            Me._NewState = Nothing
        ...
        ''' <param name="NewState">Entidade em seu estado atual</param>
        ''' <remarks></remarks>
        Public Sub New(ByRef Reactors As Reactors, OriginalState As Entity, NewState As Entity)
